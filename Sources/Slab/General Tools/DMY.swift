...
extension DMY {
    ...
    var isTomorrow: Bool { self == .today >> 1.day }
    var isYesterday: Bool { self == .today << 1.day }
    var isPast: Bool { self < .today }
    var isNotPast: Bool { self >= .today }
    var isFuture: Bool { self > .today }
    var isNotFuture: Bool { self <= .today }
...
extension DMY {
    static var today: DMY {
        DMY(Date())
    }
    
    static var distantPast: DMY {
        DMY(y: 0, m: 1, d: 1)
    }
    
    static var distantFuture: DMY {
...
extension DMY {
    ...
    func formatted(_ formats: Format...) -> String {
...
extension DMY: Equatable, Comparable, Hashable, Identifiable {
    static func < (lhs: DMY, rhs: DMY) -> Bool {
...
extension DMY {
    ...
    init?(dmy: String, separator: String = "/") {
...
struct DMYPicker: View {
    @Binding var selection: DMY
    @State private var date: Date
    
    init(selection: Binding<DMY>) {
