...
extension FileManager {
    @inlinable public func fileExists(at url: URL) -> Bool {
        fileExists(atPath: url.path)
    }
    
    @inlinable public func directoryExists(atPath path: String) -> Bool {
        var isDir: ObjCBool = false
        guard fileExists(atPath: path, isDirectory: &isDir) else { return false }
        return isDir.boolValue
    }
    
    @inlinable public func directoryExists(at url: URL) -> Bool {
        directoryExists(atPath: url.path)
    }
    
    @inlinable public var cacheDirectory: URL {
        urls(for: .cachesDirectory, in: .userDomainMask).first!
    }
    
    @inlinable public var documentsDirectory: URL {
