...
extension Date {
    ...
    @inlinable public func progress(between start: Date, and end: Date) -> Double {
        guard start < end else { return 0 }
        return min(1, max(0, Date().timeIntervalSince(start) / end.timeIntervalSince(start)))
    }
    
    /// Returns the fraction of time elapsed between two dates, as a Double in the range `0...1`
    @inlinable public func progress(in interval: ClosedRange<Date>) -> Double {
    ...
    @inlinable public func at(_ time: HoursMinutes) -> Date {
    ...
    @inlinable public var daysAgo: Int {
    ...
    @inlinable public var daysFromNow: Int {
...
@inlinable public func >> (lhs: Date, rhs: TimeInterval) -> Date {
...
@inlinable public func >> (lhs: Date, rhs: DateComponents) -> Date {
...
extension ClosedRange where Bound == Date {
    ...
    @inlinable public var isPresent: Bool { contains(Date()) }
    
    /// Returns a Bool indicating whether the Date range is entirely in the past
    @inlinable public var isPast: Bool { upperBound.isPast }
    
    /// Returns a Bool indicating whether the Date range is entirely in the future
    @inlinable public var isFuture: Bool { lowerBound.isFuture }
