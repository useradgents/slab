...
curl 'https://localise.biz/api/export/archive/strings.zip?key='"${LOCALISE_API_KEY}"'&filter=!android-only&fallback='"${LOCALISE_FALLBACK_LANGUAGE+en}"'&printf=objc' --max-time 5 -s > /tmp/localise_strings.zip || exit 0
